...
class Cart extends Component {
    ...
    updateQuantity = async({productId, quantity}) => {
        ...
        const {setProductQuantityInCart: cart} = await this
            .props
...
const CartProvider = ({children}) => {
    return (
        <Connect
            query={query(GetCart, {
            id: localStorage.getItem(FORMIDABLE_CART_ID)
        })}
            mutation={{
            ...
            {({loaded, data, updateQuantity, purchaseCart}) => {
                ...
                return (
                    <Cart cart={cart} updateQuantity={updateQuantity} purchaseCart={purchaseCart}>
                        {children}
                    </Cart>
                );
            }}
